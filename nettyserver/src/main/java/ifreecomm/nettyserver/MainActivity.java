...
import ifreecomm.nettyserver.netty.R;
        ...
        switch (v.getId()) {
            ...
            case R.id.send_btn:
                if (!NettyTcpServer.getInstance().isServerStart()) {
                    Toast.makeText(getApplicationContext(), "Not connected, please connect first", LENGTH_SHORT).show();
                } else {
...
//               When the first item in the Spinner is removed, onItemSelected will not be triggered (because mSelectedPosition != mOldSelectedPosition)
                if (i == 0) {
                    try {
                        ...
                        field.setAccessible(true);  // Set mOldSelectedPosition accessible
                        field.setInt(mSpinner, AdapterView.INVALID_POSITION); // Set the value of mOldSelectedPosition
                    } catch (Exception e) {
                        e.printStackTrace();
                    }
                }
                ...
                synchronized (clientChanelArray) {
                    ...
                    runOnUiThread(() -> {
                        ...
                        Toast.makeText(MainActivity.this, clientChanel.getClientIp() + " Disconnect", Toast.LENGTH_LONG).show();
                        spinnerAdapter.notifyDataSetChanged();
                    });
                }
                return;
